...
public class Commands {
    ...
    public static void register(
            ...
            CommandRegistryAccess ignoredRegistry,
    ...
    private static int printHelp(CommandContext<ServerCommandSource> context) {
        ...
        if (player) sb.append("§5===== MC Offline Auth (Fabric) =====§r\n");
        else sb.append("===== MC Offline Auth (Fabric) =====\n");

        sb.append("/offauth help\n");

        sb.append("/offauth info.\n");
        if (privileged) sb.append("/offauth info <user>\n");

        sb.append("/offauth bind\n");
        if (privileged) sb.append("/offauth bind <user> <key>\n");

        sb.append("/offauth unbind\n");
        if (privileged) {
            sb.append("/offauth unbind <user>\n");
            if (player) sb.append("/offauth unbind -- §7// all players§r\n");
            else sb.append("/offauth unbind -- // all players\n");
        }
        ...
        if (privileged) {
            ...
            if (player) sb.append("/offauth grace -- §7// all players§r");
            else sb.append("/offauth grace -- // all players");
        } else {
            if (player) {
    ...
    private static int showStatusInfo(CommandContext<ServerCommandSource> context) {
        var src = context.getSource();
    ...
    private static int showUserInfo(CommandContext<ServerCommandSource> context) {
        var src = context.getSource();
    ...
    private static int enableMod(CommandContext<ServerCommandSource> context) {
        if (!ServerConfig.setEnforcing(true)) {
            context.getSource()
                    .sendFeedback(
                            () -> Text.literal("Authentication is already active.")
                                    .formatted(Formatting.RED),
                            false);
            return FAIL;
        }
    ...
    private static int disableMod(CommandContext<ServerCommandSource> context) {
        if (!ServerConfig.setEnforcing(false)) {
            context.getSource()
                    .sendFeedback(
                            () -> Text.literal("Authentication is already inactive.")
                                    .formatted(Formatting.RED),
                            false);
            return FAIL;
        }
    ...
    private static int reloadModConfig(CommandContext<ServerCommandSource> context) {
    ...
    private static int bindSelf(CommandContext<ServerCommandSource> context) {
    ...
    private static int bindOthers(CommandContext<ServerCommandSource> context) {
        String user = StringArgumentType.getString(context, "user");
    ...
    private static int unbindSelf(CommandContext<ServerCommandSource> context) {
        ServerPlayerEntity player = context.getSource().getPlayer();
        if (player == null) {
            context.getSource()
                    .sendFeedback(
        ...
        if (AuthorisedKeys.unbind(user, true)) {
            ...
            context.getSource()
                    .sendFeedback(
                            () -> Text.literal("You haven't bound your key yet.")
                                    .formatted(Formatting.RED),
                            false);
            return FAIL;
        }
    ...
    private static int unbindOthers(CommandContext<ServerCommandSource> context) {
        String user = StringArgumentType.getString(context, "user");
        ...
        if (AuthorisedKeys.unbind(user, true)) {
            ...
            context.getSource()
                    .sendFeedback(
                            () -> Text.literal("Unbound user %s.".formatted(user))
                                    .formatted(Formatting.GREEN),
                            true);
            if (!ServerConfig.allowsUnboundUsers())
                context.getSource()
    ...
    private static int grantGracePeriod(CommandContext<ServerCommandSource> context) {
        String user = StringArgumentType.getString(context, "user");
        MCOfflineAuth.UNBOUND_USER_GRACES.hold(user);
        if (Objects.equals(user, "--"))
            context.getSource()
    ...
    private static int ignoreUUID(CommandContext<ServerCommandSource> context) {
        UUID uuid = UuidArgumentType.getUuid(context, "uuid");
        if (!IgnoredUsers.ignoreUUID(uuid)) {
            context.getSource()
                    .sendFeedback(
                            () -> Text.literal("This UUID is already in the ignore list.")
                                    .formatted(Formatting.RED),
                            false);
            return FAIL;
        } else {
    ...
    private static int ignoreUsername(CommandContext<ServerCommandSource> context) {
    ...
    private static int unignoreUUID(CommandContext<ServerCommandSource> context) {
        UUID uuid = UuidArgumentType.getUuid(context, "uuid");
    ...
    private static int unignoreUsername(CommandContext<ServerCommandSource> context) {
        String name = StringArgumentType.getString(context, "name");
    ...
    private static int approveRequest(CommandContext<ServerCommandSource> context) {
        String name = StringArgumentType.getString(context, "name");
        if (MCOfflineAuth.KEY_CHANGE_REQUESTS.approveUser(name)) {
            ...
            LOGGER.info(
                    "{} approved bind/rebind request for {}.",
    ...
    private static int rejectRequest(CommandContext<ServerCommandSource> context) {
        String name = StringArgumentType.getString(context, "name");
    ...
    private static int listUUIDRemaps(CommandContext<ServerCommandSource> context) {
        StringBuilder listSb = new StringBuilder();

        if (context.getSource().isExecutedByPlayer())
            listSb.append("§e%s§r UUID remaps:\n".formatted(UUIDRemap.REMAPS.size()));
        else listSb.append("%s UUID remaps:\n".formatted(UUIDRemap.REMAPS.size()));
    ...
    private static int showUUIDRemap(CommandContext<ServerCommandSource> context) {
    ...
    private static int setupUUIDRemap(CommandContext<ServerCommandSource> context) {
    ...
    private static int eraseUUIDRemap(CommandContext<ServerCommandSource> context) {
        UUID src = UuidArgumentType.getUuid(context, "uuid_from");
